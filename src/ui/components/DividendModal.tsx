...
const SURPLUS_COLOR = '#8b5cf6' // Violet-500 matching Budget card
...
export function DividendDistributionModal({
    ...
    const totalToDistribute = useMemo(() => {
        const divTotal = recipients.reduce((sum, r) => sum + r.amount, 0)
        return divTotal + surplus
    }, [recipients, surplus])
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl p-0 bg-background/95 backdrop-blur-3xl overflow-hidden rounded-[2.5rem] shadow-2xl transition-all duration-500 border-[3px] border-sky-500/50 shadow-sky-500/10">
                <div ref={printRef} className="p-6 md:p-8 space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar [print-color-adjust:exact] -webkit-print-color-adjust:exact">
                    <DialogHeader className="flex flex-row items-center justify-between space-y-0 text-start">
                        <div className="flex items-center gap-4">
                            <div className="p-4 rounded-2xl shadow-inner bg-sky-500/10 text-sky-600 dark:text-sky-400">
                                <PieChartIcon className="w-6 h-6" />
                            </div>
                            <div>
                                <DialogTitle className="text-2xl font-black tracking-tight">{t('budget.dividends', 'Dividend Distribution')}</DialogTitle>
                                <DialogDescription className="text-sm font-semibold text-muted-foreground/80 lowercase">
                                    {t('budget.profitSharingAnalysis', 'Detailed breakdown of profit distribution')}
                                </DialogDescription>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="text-right">
                                ...
                                <div className="text-3xl font-black tabular-nums tracking-tighter text-sky-600 dark:text-sky-400 leading-none">
                                    {formatCurrency(totalToDistribute, baseCurrency as any, iqdPreference)}
                                </div>
                            </div>
                            <Button
                                variant="outline"
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={(value: any) => formatCurrency(value, baseCurrency as any, iqdPreference)}
                                    />
                                    <Legend
                            ...
                            <Card className="bg-sky-500/5 border-sky-500/10 rounded-3xl overflow-hidden">
                                <CardContent className="p-5 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-2xl font-black tabular-nums tracking-tight">{formatCurrency(totalToDistribute - surplus, baseCurrency as any, iqdPreference)}</div>
                                    </div>
                                </CardContent>
                            </Card>
                                    ...
                                    {recipients.map((recipient, idx) => (
                                        <TableRow key={idx} className="hover:bg-primary/5 transition-colors border-border/40 group">
                                            ...
                                            <TableCell className={cn("px-8 tabular-nums font-black text-sky-600 dark:text-sky-400 text-lg", isRtl ? "text-left" : "text-right")}>
                                                {formatCurrency(recipient.amount, recipient.currency as any, iqdPreference)}
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                    {surplus > 0 && (
                                        <TableRow className="bg-violet-500/5 hover:bg-violet-500/10 transition-colors border-t-2 border-violet-500/20">
                                            ...
                                            <TableCell className={cn("px-8 tabular-nums font-black text-violet-700 dark:text-violet-300 text-lg", isRtl ? "text-left" : "text-right")}>
                                                {formatCurrency(surplus, baseCurrency as any, iqdPreference)}
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
