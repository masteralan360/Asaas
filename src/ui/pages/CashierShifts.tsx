...
import { Ban, CalendarClock, Check, CircleCheck, ClipboardCheck, Clock3, FileText, PauseCircle, Pencil, Plus, ShieldCheck, UserRound, UsersRound, WalletCards, X } from 'lucide-react'
...
function statusClass(status: ShiftDisplayStatus) {
  switch (status) {
    ...
    case 'paused': return 'border-amber-500/20 bg-amber-500/10 text-amber-700 dark:text-amber-300'
    case 'completed': return 'border-slate-500/20 bg-slate-500/10 text-muted-foreground'
    case 'terminated': return 'border-rose-500/20 bg-rose-500/10 text-rose-700 dark:text-rose-300'
...
export function CashierShifts() {
  ...
  const hasValidCashDrawer = Boolean(selectedCashDrawer
  ...
  const hasActiveShift = occurrences.some(
    (occurrence) => occurrence.cashierUserId === currentUserId && occurrence.status === 'active',
  )
  ...
  const shiftRows = useMemo(() => {
    return getCashierShiftListRows({ assignments, occurrences, cashierUserId: currentUserId, now })
  ...
  const teamShiftRows = useMemo(() => getCashierShiftTeamRows({ assignments, occurrences })
    .filter((row) => isDateInDateRange(row.scheduledStartAt, dateRange, customDates)),
  ...
  const assignmentDurationMinutes = assignmentStartTime && assignmentEndTime
    ? ((assignmentEndTime.getHours() * 60 + assignmentEndTime.getMinutes())
  ...
  const openEditAssignmentDialog = (assignment: CashierShiftAssignment) => {
    ...
    setSelectedTemplateId(templates.some((template) => template.id === assignment.templateId && template.isActive)
      ? assignment.templateId!
      : '__custom__')
    setAssignmentStartTime(timeDate(assignment.startTime))
    setAssignmentEndTime(timeDate(assignment.endTime))
  ...
  const saveAssignment = async () => {
    if (!workspaceId || !hasValidCashDrawer || !selectedCashDrawer || !selectedCashier || !assignmentStartTime || !assignmentEndTime || workingDays.length === 0) return
    const parsedEarlyFinishOffsetMinutes = earlyFinishPolicy === 'time_before_end'
      ? Number(earlyFinishOffsetMinutes)
      : null
    ...
    try {
      const input = {
        ...
        workingDays,
        earlyFinishPolicy,
        earlyFinishOffsetMinutes: parsedEarlyFinishOffsetMinutes,
      }
  ...
  const startShift = async () => {
    ...
    if (hasActiveShift) {
      ...
      toast({ title: t('common.error'), description: t('paymentAccounts.activeShiftMustBeCompleted'), variant: 'destructive' })
      return
    }
    setStartingShift(true)
    try {
      await startCashierShiftOccurrence(workspaceId, { assignmentId: assignment.id, cashierUserId: currentUserId, scheduledStartAt: startDialogRow.scheduledStartAt })
  ...
  const completeShift = async () => {
    ...
    try {
      ...
      toast({ title: t('common.error'), description: t('paymentAccounts.completeShiftFailed'), variant: 'destructive' })
    } finally {
  ...
  const isEarlyFinishOffsetValid = earlyFinishPolicy !== 'time_before_end'
  ...
  const pendingEarlyFinishRequests = user?.role === 'admin'
    ? occurrences.filter((occurrence) => occurrence.status === 'active'
      && occurrence.earlyFinishPolicy === 'request_approval'
      && occurrence.earlyFinishRequestStatus === 'requested')
    : []
  const completeShiftRequiresReason = Boolean(completeDialogRow?.occurrence
  ...
  return (
    <div className="space-y-8 p-6">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'shifts' | 'team' | 'assignments')} className="space-y-6">
        <TabsList className={`grid h-auto min-h-12 w-full ${isAdmin ? 'max-w-2xl grid-cols-3' : 'max-w-md grid-cols-2'} rounded-2xl bg-secondary/50 p-1`}>
          <TabsTrigger value="shifts" className="gap-2 rounded-xl"><Clock3 className="h-4 w-4" />{t('paymentAccounts.shiftsTab')}</TabsTrigger>
          {isAdmin ? <TabsTrigger value="team" className="gap-2 rounded-xl"><UsersRound className="h-4 w-4" />{t('paymentAccounts.teamShifts')}</TabsTrigger> : null}
        ...
        <TabsContent value="shifts" className="mt-0 space-y-6">
          <Card><CardHeader className="border-b border-border/60 pb-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>{t('paymentAccounts.myShifts')}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.myShiftsDescription')}</p></div><span className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{t('paymentAccounts.shiftCount', { count: shiftRows.length })}</span></div></CardHeader>
            <CardContent className="pt-6"><DateRangeFilters /><div className="mt-6 overflow-x-auto rounded-2xl border border-border/60"><Table><TableHeader><TableRow><TableHead>{t('paymentAccounts.shift')}</TableHead><TableHead>{t('paymentAccounts.cashDrawer')}</TableHead><TableHead>{t('paymentAccounts.scheduledStart')}</TableHead><TableHead>{t('paymentAccounts.scheduledEnd')}</TableHead><TableHead>{t('common.status')}</TableHead><TableHead className="text-end">{t('common.actions')}</TableHead></TableRow></TableHeader><TableBody>
              {shiftRows.length === 0 ? <TableRow><TableCell colSpan={6} className="py-12 text-center"><CalendarClock className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><p className="font-semibold">{t('paymentAccounts.noAssignedShifts')}</p><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.noAssignedShiftsDescription')}</p></TableCell></TableRow> : shiftRows.map((row) => {
                const occurrence = row.occurrence ?? null
                const completionEligibility = occurrence ? getCashierShiftCompletionEligibility(occurrence, now) : null
                const canRequestEarlyFinish = occurrence
                  && occurrence.status === 'active'
                  && occurrence.earlyFinishPolicy === 'request_approval'
                  && occurrence.earlyFinishRequestStatus === 'not_requested'
                  && now < new Date(occurrence.scheduledEndAt)
                const requestPending = occurrence?.earlyFinishPolicy === 'request_approval'
                  && occurrence.earlyFinishRequestStatus === 'requested'
                const requestRejected = occurrence?.earlyFinishPolicy === 'request_approval'
                  && occurrence.earlyFinishRequestStatus === 'rejected'

                return <TableRow key={row.key}>
                  <TableCell className="font-medium">{occurrence?.templateNameSnapshot || row.assignment?.templateNameSnapshot || t('paymentAccounts.customShift')}</TableCell>
                  <TableCell>{occurrence?.accountNameSnapshot || row.assignment?.accountNameSnapshot || '—'}</TableCell>
                  <TableCell>{formatOccurrenceDateTime(row.scheduledStartAt)}</TableCell>
                  <TableCell>{formatOccurrenceDateTime(row.scheduledEndAt)}</TableCell>
                  <TableCell><span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(row.status)}`}>{statusIcon(row.status)}{t(`paymentAccounts.shiftStatuses.${row.status}`)}</span></TableCell>
                  <TableCell className="text-end"><div className="flex flex-wrap justify-end gap-2">
                    {occurrence ? <Button size="sm" variant="outline" onClick={() => navigate(`/payment-accounts/cashier-shifts/${occurrence.id}`)}><FileText className="mr-2 h-4 w-4" />{t('paymentAccounts.viewShiftDetails')}</Button> : null}
                    {row.status === 'available' ? <Button size="sm" disabled={hasActiveShift} title={hasActiveShift ? t('paymentAccounts.activeShiftMustBeCompleted') : undefined} onClick={() => setStartDialogRow(row)}><ShieldCheck className="mr-2 h-4 w-4" />{t('paymentAccounts.startShift')}</Button> : null}
                    {row.status === 'active' && completionEligibility?.canComplete ? <Button size="sm" onClick={() => openCompleteShiftDialog(row)}><CircleCheck className="mr-2 h-4 w-4" />{t('paymentAccounts.completeShift')}</Button> : null}
                    {row.status === 'active' && canRequestEarlyFinish ? <Button size="sm" variant="outline" onClick={() => openEarlyFinishRequestDialog(row)}><ClipboardCheck className="mr-2 h-4 w-4" />{t('paymentAccounts.requestEarlyFinish')}</Button> : null}
                    {row.status === 'active' && requestPending ? <span className="self-center rounded-full border border-amber-500/20 bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-700 dark:text-amber-300">{t('paymentAccounts.earlyFinishRequestStatuses.requested')}</span> : null}
                    {row.status === 'active' && requestRejected ? <span className="self-center rounded-full border border-rose-500/20 bg-rose-500/10 px-2.5 py-1 text-xs font-semibold text-rose-700 dark:text-rose-300">{t('paymentAccounts.earlyFinishRequestStatuses.rejected')}</span> : null}
                  </div></TableCell>
                </TableRow>
              })}
        ...
        {isAdmin ? <TabsContent value="team" className="mt-0 space-y-6">
          <Card><CardHeader className="border-b border-border/60 pb-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><UsersRound className="h-5 w-5 text-primary" />{t('paymentAccounts.teamShifts')}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.teamShiftsDescription')}</p></div><span className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{t('paymentAccounts.shiftCount', { count: teamShiftRows.length })}</span></div></CardHeader>
            <CardContent className="pt-6"><DateRangeFilters /><div className="mt-6 overflow-x-auto rounded-2xl border border-border/60"><Table><TableHeader><TableRow><TableHead>{t('paymentAccounts.cashier')}</TableHead><TableHead>{t('paymentAccounts.shift')}</TableHead><TableHead>{t('paymentAccounts.cashDrawer')}</TableHead><TableHead>{t('paymentAccounts.actualStart')}</TableHead><TableHead>{t('paymentAccounts.scheduledEnd')}</TableHead><TableHead>{t('common.status')}</TableHead><TableHead className="text-end">{t('common.actions')}</TableHead></TableRow></TableHeader><TableBody>{teamShiftRows.length === 0 ? <TableRow><TableCell colSpan={7} className="py-12 text-center"><UsersRound className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><p className="font-semibold">{t('paymentAccounts.noTeamShifts')}</p><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.noTeamShiftsDescription')}</p></TableCell></TableRow> : teamShiftRows.map((row) => {
              const occurrence = row.occurrence!
              return <TableRow key={row.key}><TableCell className="font-medium">{occurrence.cashierNameSnapshot}</TableCell><TableCell>{occurrence.templateNameSnapshot || t('paymentAccounts.customShift')}</TableCell><TableCell>{occurrence.accountNameSnapshot}</TableCell><TableCell>{formatOccurrenceDateTime(occurrence.startedAt)}</TableCell><TableCell>{formatOccurrenceDateTime(occurrence.scheduledEndAt)}</TableCell><TableCell><span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(row.status)}`}>{statusIcon(row.status)}{t(`paymentAccounts.shiftStatuses.${row.status}`)}</span></TableCell><TableCell className="text-end"><Button size="sm" variant="outline" onClick={() => navigate(`/payment-accounts/cashier-shifts/${occurrence.id}`)}><FileText className="mr-2 h-4 w-4" />{t('paymentAccounts.viewShiftDetails')}</Button></TableCell></TableRow>
            })}</TableBody></Table></div></CardContent></Card>
        </TabsContent> : null}
        ...
        <TabsContent value="assignments" className="mt-0 space-y-6">
          ...
          <Card><CardHeader className="border-b border-border/60 pb-5"><div><CardTitle>{t('paymentAccounts.shiftAssignments')}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.shiftAssignmentsDescription')}</p></div></CardHeader><CardContent className="pt-6"><div className="overflow-x-auto rounded-2xl border border-border/60"><Table><TableHeader><TableRow><TableHead>{t('paymentAccounts.cashier')}</TableHead><TableHead>{t('paymentAccounts.shift')}</TableHead><TableHead>{t('paymentAccounts.cashDrawer')}</TableHead><TableHead>{t('paymentAccounts.shiftTime')}</TableHead><TableHead>{t('paymentAccounts.earlyFinishRule')}</TableHead><TableHead>{t('paymentAccounts.workingDays')}</TableHead><TableHead className="text-end">{t('common.actions')}</TableHead></TableRow></TableHeader><TableBody>
            {visibleAssignments.length === 0 ? <TableRow><TableCell colSpan={7} className="py-12 text-center"><UserRound className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><p className="font-semibold">{t('paymentAccounts.noShiftAssignments')}</p><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.noShiftAssignmentsDescription')}</p></TableCell></TableRow> : visibleAssignments.map((assignment) => <TableRow key={assignment.id}><TableCell className="font-medium">{assignment.cashierNameSnapshot}{assignment.cashierUserId === currentUserId ? <span className="ml-2 rounded-full border border-primary/20 bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">{t('paymentAccounts.you')}</span> : null}</TableCell><TableCell>{assignment.templateNameSnapshot || t('paymentAccounts.customShift')}</TableCell><TableCell>{assignment.accountNameSnapshot}</TableCell><TableCell>{assignment.startTime} — {assignment.endTime}{isOvernight(assignment.startTime, assignment.endTime) ? <span className="ml-2 text-xs text-muted-foreground">{t('paymentAccounts.overnight')}</span> : null}</TableCell><TableCell>{t(`paymentAccounts.earlyFinishPolicies.${assignment.earlyFinishPolicy ?? 'scheduled_end'}`)}</TableCell><TableCell>{workingDayList(assignment.workingDays)}</TableCell><TableCell className="text-end"><Button type="button" size="sm" variant="outline" onClick={() => openEditAssignmentDialog(assignment)}><Pencil className="mr-2 h-4 w-4" />{t('paymentAccounts.editCashierShift')}</Button></TableCell></TableRow>)}
          </TableBody></Table></div></CardContent></Card>
          {user?.role === 'admin' ? <Card><CardHeader className="border-b border-border/60 pb-5"><CardTitle className="flex items-center gap-2"><ClipboardCheck className="h-5 w-5 text-primary" />{t('paymentAccounts.earlyFinishRequests')}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.earlyFinishRequestsDescription')}</p></CardHeader><CardContent className="pt-6"><div className="overflow-x-auto rounded-2xl border border-border/60"><Table><TableHeader><TableRow><TableHead>{t('paymentAccounts.cashier')}</TableHead><TableHead>{t('paymentAccounts.shift')}</TableHead><TableHead>{t('paymentAccounts.earlyFinishReason')}</TableHead><TableHead>{t('paymentAccounts.requestedAt')}</TableHead><TableHead className="text-end">{t('common.actions')}</TableHead></TableRow></TableHeader><TableBody>{pendingEarlyFinishRequests.length === 0 ? <TableRow><TableCell colSpan={5} className="py-10 text-center text-sm text-muted-foreground">{t('paymentAccounts.noEarlyFinishRequests')}</TableCell></TableRow> : pendingEarlyFinishRequests.map((occurrence) => <TableRow key={occurrence.id}><TableCell className="font-medium">{occurrence.cashierNameSnapshot}</TableCell><TableCell>{occurrence.templateNameSnapshot || t('paymentAccounts.customShift')}</TableCell><TableCell className="max-w-72 truncate" title={occurrence.earlyFinishRequestReason || undefined}>{occurrence.earlyFinishRequestReason}</TableCell><TableCell>{formatOccurrenceDateTime(occurrence.earlyFinishRequestedAt || occurrence.updatedAt)}</TableCell><TableCell className="text-end"><Button size="sm" variant="outline" onClick={() => navigate(`/payment-accounts/cashier-shifts/${occurrence.id}`)}><FileText className="mr-2 h-4 w-4" />{t('paymentAccounts.reviewEarlyFinishRequest')}</Button></TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card> : null}
        </TabsContent>
      </Tabs>
      ...
      <AppDialog open={assignmentDialogOpen} onOpenChange={(open) => { if (!savingAssignment) { setAssignmentDialogOpen(open); if (!open) setEditingAssignment(null) } }}><AppDialogContent className="max-w-3xl" showCloseButton={!savingAssignment} onPointerDownOutside={(event) => savingAssignment && event.preventDefault()} onEscapeKeyDown={(event) => savingAssignment && event.preventDefault()}><AppDialogHeader><AppDialogTitle>{t(editingAssignment ? 'paymentAccounts.editCashierShift' : 'paymentAccounts.assignCashierShift')}</AppDialogTitle><AppDialogDescription>{t(editingAssignment ? 'paymentAccounts.editCashierShiftDescription' : 'paymentAccounts.assignCashierShiftDescription')}</AppDialogDescription></AppDialogHeader><AppDialogBody><div className="grid gap-6">
        <section className="grid gap-4"><div><h3 className="font-semibold">{t('paymentAccounts.cashier')}</h3><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.cashierAssignmentDescription')}</p></div><div className="grid gap-4 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="cashier-shift-member">{t('paymentAccounts.assignedWorkspaceMember')}</Label><Select value={cashierUserId} onValueChange={setCashierUserId} disabled={savingAssignment || workspaceMembers.length === 0}><SelectTrigger id="cashier-shift-member"><SelectValue placeholder={t('paymentAccounts.selectWorkspaceMember')} /></SelectTrigger><SelectContent>{workspaceMembers.map((member) => <SelectItem key={member.id} value={member.id}>{member.name || member.email}</SelectItem>)}</SelectContent></Select></div><PaymentAccountSelector workspaceId={workspaceId} value={cashDrawerId} onValueChange={(account) => setCashDrawerId(account?.id ?? null)} cashDrawerOnly allowNoAccount={false} applyDefault={false} placeholder={t('paymentAccounts.selectCashDrawer')} disabled={savingAssignment} label={t('paymentAccounts.cashDrawer')} /></div></section>
        ...
        <section className="grid gap-4 border-t border-border/60 pt-6"><div><h3 className="font-semibold">{t('paymentAccounts.earlyFinishRule')}</h3><p className="mt-1 text-sm text-muted-foreground">{t('paymentAccounts.earlyFinishRuleDescription')}</p></div><div className="grid gap-4 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="cashier-shift-early-finish-policy">{t('paymentAccounts.earlyFinishRule')}</Label><Select value={earlyFinishPolicy} onValueChange={(value) => setEarlyFinishPolicy(value as CashierShiftEarlyFinishPolicy)} disabled={savingAssignment}><SelectTrigger id="cashier-shift-early-finish-policy"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="scheduled_end">{t('paymentAccounts.earlyFinishPolicies.scheduled_end')}</SelectItem><SelectItem value="time_before_end">{t('paymentAccounts.earlyFinishPolicies.time_before_end')}</SelectItem><SelectItem value="request_approval">{t('paymentAccounts.earlyFinishPolicies.request_approval')}</SelectItem><SelectItem value="free_with_reason">{t('paymentAccounts.earlyFinishPolicies.free_with_reason')}</SelectItem></SelectContent></Select></div>{earlyFinishPolicy === 'time_before_end' ? <div className="grid gap-2"><Label htmlFor="cashier-shift-early-finish-offset">{t('paymentAccounts.earlyFinishOffsetMinutes')}</Label><Input id="cashier-shift-early-finish-offset" type="number" min="1" inputMode="numeric" value={earlyFinishOffsetMinutes} onChange={(event) => setEarlyFinishOffsetMinutes(event.target.value)} disabled={savingAssignment} placeholder="0" /><p className="text-xs text-muted-foreground">{t('paymentAccounts.earlyFinishOffsetHint')}</p></div> : <div className="rounded-2xl border border-border/60 bg-muted/30 p-4 text-sm text-muted-foreground">{t(`paymentAccounts.earlyFinishPolicyDescriptions.${earlyFinishPolicy}`)}</div>}</div></section>
      ...
      </div></AppDialogBody><AppDialogFooter><Button variant="outline" onClick={() => { setAssignmentDialogOpen(false); setEditingAssignment(null) }} disabled={savingAssignment}>{t('common.cancel')}</Button><Button onClick={saveAssignment} disabled={savingAssignment || !hasValidCashDrawer || !selectedCashier || !assignmentStartTime || !assignmentEndTime || workingDays.length === 0 || assignmentStartKey === assignmentEndKey || !isEarlyFinishOffsetValid}>{editingAssignment ? <Pencil className="mr-2 h-4 w-4" /> : <UserRound className="mr-2 h-4 w-4" />}{t(editingAssignment ? 'paymentAccounts.editCashierShift' : 'paymentAccounts.assignCashierShift')}</Button></AppDialogFooter></AppDialogContent></AppDialog>

      <AppDialog open={Boolean(startDialogRow)} onOpenChange={(open) => !startingShift && !open && setStartDialogRow(null)}><AppDialogContent className="max-w-xl" showCloseButton={!startingShift} onPointerDownOutside={(event) => startingShift && event.preventDefault()} onEscapeKeyDown={(event) => startingShift && event.preventDefault()}><AppDialogHeader><AppDialogTitle>{t('paymentAccounts.startShift')}</AppDialogTitle><AppDialogDescription>{t('paymentAccounts.startShiftConfirmationDescription')}</AppDialogDescription></AppDialogHeader><AppDialogBody>{startDialogRow ? <div className="grid gap-4"><div className="rounded-2xl border border-border/60 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('paymentAccounts.yourShiftWillStart')}</p><p className="mt-2 font-semibold">{formatOccurrenceDateTime(startDialogRow.scheduledStartAt)}</p></div><div className="rounded-2xl border border-border/60 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('paymentAccounts.yourShiftWillEnd')}</p><p className="mt-2 font-semibold">{formatOccurrenceDateTime(startDialogRow.scheduledEndAt)}</p></div></div> : null}</AppDialogBody><AppDialogFooter><Button variant="outline" onClick={() => setStartDialogRow(null)} disabled={startingShift}>{t('common.cancel')}</Button><PressAndHoldButton onComplete={startShift} idleLabel={t('paymentAccounts.holdToStartShift')} holdingLabel={t('paymentAccounts.keepHoldingToStartShift')} loadingLabel={t('paymentAccounts.startingShift')} isLoading={startingShift} disabled={hasActiveShift} icon={<ShieldCheck className="h-4 w-4" />} /></AppDialogFooter></AppDialogContent></AppDialog>

      <AppDialog open={Boolean(completeDialogRow)} onOpenChange={(open) => !completingShift && !open && setCompleteDialogRow(null)}><AppDialogContent className="max-w-xl" showCloseButton={!completingShift} onPointerDownOutside={(event) => completingShift && event.preventDefault()} onEscapeKeyDown={(event) => completingShift && event.preventDefault()}><AppDialogHeader><AppDialogTitle>{t('paymentAccounts.completeShift')}</AppDialogTitle><AppDialogDescription>{completeShiftRequiresReason ? t('paymentAccounts.earlyFinishReasonDescription') : t('paymentAccounts.completeShiftConfirmationDescription')}</AppDialogDescription></AppDialogHeader><AppDialogBody>{completeDialogRow ? <div className="grid gap-4"><div className="rounded-2xl border border-border/60 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('paymentAccounts.scheduledEnd')}</p><p className="mt-2 font-semibold">{formatOccurrenceDateTime(completeDialogRow.scheduledEndAt)}</p></div>{completeShiftRequiresReason ? <div className="grid gap-2"><Label htmlFor="cashier-shift-completion-reason">{t('paymentAccounts.earlyFinishReason')}</Label><Textarea id="cashier-shift-completion-reason" value={completionReason} onChange={(event) => setCompletionReason(event.target.value)} disabled={completingShift} /></div> : null}</div> : null}</AppDialogBody><AppDialogFooter><Button variant="outline" onClick={() => setCompleteDialogRow(null)} disabled={completingShift}>{t('common.cancel')}</Button><PressAndHoldButton onComplete={completeShift} idleLabel={t('paymentAccounts.holdToCompleteShift')} holdingLabel={t('paymentAccounts.keepHoldingToCompleteShift')} loadingLabel={t('paymentAccounts.completingShift')} isLoading={completingShift} disabled={completeShiftRequiresReason && !completionReason.trim()} icon={<CircleCheck className="h-4 w-4" />} /></AppDialogFooter></AppDialogContent></AppDialog>

      <AppDialog open={Boolean(earlyFinishRequestRow)} onOpenChange={(open) => !requestingEarlyFinish && !open && setEarlyFinishRequestRow(null)}><AppDialogContent className="max-w-xl" showCloseButton={!requestingEarlyFinish} onPointerDownOutside={(event) => requestingEarlyFinish && event.preventDefault()} onEscapeKeyDown={(event) => requestingEarlyFinish && event.preventDefault()}><AppDialogHeader><AppDialogTitle>{t('paymentAccounts.requestEarlyFinish')}</AppDialogTitle><AppDialogDescription>{t('paymentAccounts.requestEarlyFinishDescription')}</AppDialogDescription></AppDialogHeader><AppDialogBody><div className="grid gap-2"><Label htmlFor="cashier-shift-early-finish-request-reason">{t('paymentAccounts.earlyFinishReason')}</Label><Textarea id="cashier-shift-early-finish-request-reason" value={earlyFinishRequestReason} onChange={(event) => setEarlyFinishRequestReason(event.target.value)} disabled={requestingEarlyFinish} /></div></AppDialogBody><AppDialogFooter><Button variant="outline" onClick={() => setEarlyFinishRequestRow(null)} disabled={requestingEarlyFinish}>{t('common.cancel')}</Button><PressAndHoldButton onComplete={requestEarlyFinish} idleLabel={t('paymentAccounts.holdToRequestEarlyFinish')} holdingLabel={t('paymentAccounts.keepHoldingToRequestEarlyFinish')} loadingLabel={t('paymentAccounts.requestingEarlyFinish')} isLoading={requestingEarlyFinish} disabled={!earlyFinishRequestReason.trim()} icon={<ClipboardCheck className="h-4 w-4" />} /></AppDialogFooter></AppDialogContent></AppDialog>
    </div>
  )
}
