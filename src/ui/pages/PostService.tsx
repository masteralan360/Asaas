...
import { Banknote, CheckCircle2, CircleDollarSign, ClipboardList, History, PackageCheck, Plus, Route, Send, Store, Truck, Undo2, WalletCards } from "lucide-react";

import { useAuth } from "@/auth";
import {
  closeDeliveryRun, createDeliveryMerchantProfile, createDeliveryRun, createDeliveryShipment, payDeliveryMerchant,
  settleDeliveryCourier, updateDeliveryShipmentStatus, useAgents, useBusinessPartners, useCourierDeliveryBalances,
  useDeliveryMerchantProfiles, useDeliveryRuns, useDeliverySettlements, useDeliveryShipments, useFleetVehicles,
  useMerchantDeliveryBalances, type CurrencyCode, type DeliveryShipment, type DeliveryShipmentStatus, type WorkspacePaymentMethod,
} from "@/local-db";
...
export function PostService() {
  ...
  const partners = useBusinessPartners(workspaceId, { includeAgentRoles: false });
  ...
  const merchantCandidates = partners.filter((partner) => !merchantProfiles.some((profile) => profile.businessPartnerId === partner.id));
  ...
  async function handleCreateMerchant(event: FormEvent) {
    event.preventDefault(); if (!workspaceId || !merchantId) return; setIsSubmitting(true);
    try {
      await createDeliveryMerchantProfile(workspaceId, { businessPartnerId: merchantId, defaultFeeAmount: parseFormattedNumber(merchantFee || "0"), defaultFeePayer: merchantFeePayer, payoutSchedule: merchantPayoutSchedule });
      toast({ title: t("postService.messages.merchantEnabled"), description: t("postService.messages.merchantEnabledDescription") });
      setMerchantDialogOpen(false); setMerchantId(""); setMerchantFee("");
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="posts"><TabsList className="h-auto w-full flex-wrap justify-start gap-1 sm:w-auto"><TabsTrigger value="posts"><ClipboardList className="mr-2 h-4 w-4" />{t("postService.tabs.posts")}</TabsTrigger><TabsTrigger value="dispatch"><Send className="mr-2 h-4 w-4" />{t("postService.tabs.dispatch")}</TabsTrigger><TabsTrigger value="my-deliveries"><Route className="mr-2 h-4 w-4" />{t("postService.tabs.myDeliveries")}</TabsTrigger><TabsTrigger value="merchants"><Store className="mr-2 h-4 w-4" />{t("postService.tabs.merchants")}</TabsTrigger><TabsTrigger value="settlements"><Banknote className="mr-2 h-4 w-4" />{t("postService.tabs.settlements")}</TabsTrigger></TabsList>
      ...
      <TabsContent value="merchants" className="mt-4"><Card><CardHeader className="flex-row items-center justify-between"><CardTitle>{t("postService.cards.deliveryMerchants")}</CardTitle>{isEditor && <Button size="sm" onClick={() => setMerchantDialogOpen(true)}><Plus className="mr-2 h-4 w-4" />{t("postService.actions.enableMerchant")}</Button>}</CardHeader><CardContent className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>{t("postService.table.merchant")}</TableHead><TableHead>{t("postService.table.defaultFee")}</TableHead><TableHead>{t("postService.table.feePayer")}</TableHead><TableHead>{t("postService.table.payoutSchedule")}</TableHead><TableHead>{t("postService.table.status")}</TableHead></TableRow></TableHeader><TableBody>{merchantProfiles.length === 0 ? <EmptyRow columns={5} label={t("postService.empty.noMerchants")} /> : merchantProfiles.map((profile) => <TableRow key={profile.id}><TableCell className="font-medium">{profileNameById.get(profile.id)}</TableCell><TableCell>{formatCurrency(profile.defaultFeeAmount, features.default_currency, features.iqd_display_preference)}</TableCell><TableCell>{t(`postService.feePayer.${profile.defaultFeePayer}`)}</TableCell><TableCell>{t(`postService.payoutSchedule.${profile.payoutSchedule}`)}</TableCell><TableCell><Badge variant="outline" className={profile.isActive ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700" : "text-muted-foreground"}>{t(profile.isActive ? "postService.active" : "postService.inactive")}</Badge></TableCell></TableRow>)}</TableBody></Table></CardContent></Card></TabsContent>
      ...
      <DialogContent layout="structured" className="sm:max-w-lg">
        ...
        <form onSubmit={handleCreateMerchant} className="flex min-h-0 flex-1 flex-col">
          <DialogBody className="grid gap-4 py-5">
            <Field label={t("postService.form.businessPartner")}>
              <Select value={merchantId} onValueChange={setMerchantId}>
                <SelectTrigger><SelectValue placeholder={t("postService.placeholders.selectMerchantOrShop")} /></SelectTrigger>
                <SelectContent>
                  {merchantCandidates.map((partner) => <SelectItem key={partner.id} value={partner.id}>{partner.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
          ...
          <DialogFooter layout="structured">
            ...
            <Button type="submit" className="w-full sm:w-auto" disabled={isSubmitting || !merchantId || !merchantFee}>{t("postService.actions.enableMerchant")}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
