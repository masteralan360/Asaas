...
describe('getCashierShiftListRows', () => {
  beforeAll(async () => {
    ...
    ;({ getCashierShiftListRows, getCashierShiftTeamRows, getCashierShiftOccurrenceBounds } = await import('./paymentAccounts'))
  })
  ...
  it('shows one persisted active shift without creating speculative recurring rows', () => {
    ...
    expect(rows[0]).toMatchObject({ status: 'active', scheduledStartAt: bounds.start.toISOString() })
  })
  ...
  it('matches equivalent persisted offset timestamps with the current schedule', () => {
    ...
    expect(rows).toHaveLength(1)
    expect(rows[0]).toMatchObject({ status: 'active', scheduledStartAt: offsetTimestamp })
  })
  ...
  it('shows the current unstarted shift as ready, but not past or future schedule placeholders', () => {
    ...
    const shift = assignment()
    const bounds = getCashierShiftOccurrenceBounds(shift, now)!
    const rows = getCashierShiftListRows({ assignments: [shift], occurrences: [], cashierUserId: CASHIER_ID, now })

    expect(rows).toEqual([expect.objectContaining({ status: 'available', scheduledStartAt: bounds.start.toISOString() })])
  })
  ...
  it('retains completed history without adding an unavailable shift after the scheduled end', () => {
    ...
    const rows = getCashierShiftListRows({
      assignments: [shift],
      occurrences: [occurrence(bounds.start.toISOString(), bounds.end.toISOString(), { status: 'completed', completedAt: bounds.end.toISOString() })],
      cashierUserId: CASHIER_ID,
      now,
    })

    expect(rows).toEqual([expect.objectContaining({ status: 'completed', scheduledStartAt: bounds.start.toISOString() })])
  })
  ...
  it('makes an overnight shift ready after midnight based on the prior working day', () => {
    ...
    const shift = assignment({ startTime: '23:00', endTime: '02:00', workingDays: [previousDay.getDay()] })
  ...
  it('lists only real team occurrences, including paused and terminated snapshots without their current assignment', () => {
    ...
    const rows = getCashierShiftTeamRows({
      assignments: [],
      occurrences: [
        occurrence(bounds.start.toISOString(), bounds.end.toISOString(), { id: 'paused', status: 'paused' }),
        occurrence(new Date(bounds.start.getTime() - 86_400_000).toISOString(), new Date(bounds.end.getTime() - 86_400_000).toISOString(), {
          id: 'terminated', status: 'terminated', terminatedAt: now.toISOString(), terminatedBy: 'admin-1', terminationReason: 'Audit close',
        }),
      ],
    })
