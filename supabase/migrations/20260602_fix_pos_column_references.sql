-- Migration 20260525 dropped the `pos` column from workspaces.
-- Replace direct `SELECT pos` with override-aware workspace_module_allowed().
...
DROP FUNCTION IF EXISTS public.check_return_permissions(uuid, uuid);

CREATE FUNCTION public.check_return_permissions(p_sale_id uuid, p_user_id uuid)
