-- A Jumla delivery entry is display metadata, not a marketplace item. Keep it
-- out of pending-order notification counts without changing generic orders.
...
BEGIN
  ...
  WHERE COALESCE(item.value ->> 'metadata_type', '') <> 'jumla_khaleej_delivery_fee';
