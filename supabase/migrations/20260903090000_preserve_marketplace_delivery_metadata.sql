-- Delivery information for the Jumla Khaleej storefront lives in the existing
-- marketplace_orders.items JSONB column as non-product metadata. Preserve any
-- such entries whenever staff reduce or remove product lines. They must not
-- influence the recalculated product subtotal or total.
...
BEGIN
    ...
    LOOP
      ...
      THEN
        ...
        IF v_found_count = 1 THEN
          ...
          v_new_product_count := v_new_product_count + 1;
        END IF;
      END IF;
    END LOOP;
  ...
  -- Keep non-product JSON metadata (including the Jumla delivery fee) intact.
  -- It never enters v_new_subtotal and therefore cannot change order value.
  FOR v_original IN SELECT * FROM jsonb_array_elements(COALESCE(v_order.items, '[]'::jsonb))
  LOOP
    IF NULLIF(v_original->>'product_id', '') IS NULL THEN
      v_new_items := v_new_items || jsonb_build_array(v_original);
    END IF;
  END LOOP;

  IF v_new_product_count = 0 THEN
    RAISE EXCEPTION 'A marketplace order must keep at least one item';
  END IF;
