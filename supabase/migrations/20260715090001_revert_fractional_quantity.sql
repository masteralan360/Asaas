-- Revert fractional quantities for dynamic units (m², Kg)
-- Reverses migration 20260715090000_allow_fractional_quantity_for_dynamic_units.sql
-- WARNING: Only run this if NO fractional data (e.g. 25.5) has been entered yet.
-- If fractional data exists, the USING quantity::integer cast will fail.
...
ALTER TABLE public.sale_items
  ALTER COLUMN returned_quantity TYPE integer USING returned_quantity::integer;

ALTER TABLE public.sale_items
  ALTER COLUMN inventory_snapshot TYPE integer USING inventory_snapshot::integer;

-- inventory
ALTER TABLE public.inventory
  ALTER COLUMN quantity TYPE integer USING quantity::integer;

-- stock_batches (drop numeric-friendly check, revert type, restore original check)
...
-- Restore original complete_sale function (with INTEGER quantities)
...
-- Restore original process_sale_return function (with INTEGER quantities)
...
-- Restore original return_sale_items function (with INTEGER array parameter)
