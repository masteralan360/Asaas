-- Marketplace customers get the dedicated 'online_customer' partner role. The
-- delivered flow creates them with that role and matches existing partners by
-- phone/email across customer roles to avoid duplicates.
...
ALTER TABLE crm.business_partners
  ADD CONSTRAINT business_partners_role_check CHECK (
    role IN ('customer', 'supplier', 'both', 'agent', 'buyer', 'seller', 'online_customer')
  );

-- Marketplace order items carry the price book override cost captured at
-- checkout (cost_price). The delivered flow must prefer that value over the
-- native product cost so sales order lines report the correct COGS.

-- Marketplace deliveries use a dedicated payment method that exists only for
-- e-commerce orders. It is not exposed by any POS/cashier selector, so the
-- only way a sales order can carry it is through the marketplace delivered
-- flow. 'credit' is deliberately not allowed here.
...
ALTER TABLE crm.sales_orders
  ...
  ADD CONSTRAINT sales_orders_payment_method_check CHECK (
    payment_method IS NULL OR payment_method IN (
      'cash', 'fib', 'qicard', 'zaincash', 'fastpay', 'bank_transfer',
      'loan', 'installments', 'ecommerce'
    )
  );
...
BEGIN
  ...
  IF v_sales_order_id IS NULL THEN
    ...
    WHERE bp.workspace_id = v_order.workspace_id
      ...
      AND bp.role IN ('customer', 'both', 'online_customer')
      AND (
    ...
    LOOP
      ...
      -- Prefer the price book override cost captured at checkout; fall back
      -- to the native product cost when no override applied.
      v_product_cost_price := COALESCE(
