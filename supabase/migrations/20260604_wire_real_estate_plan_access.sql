-- Real Estate is now controlled only by explicit workspace access overrides.
...
SET real_estate = public.workspace_module_allowed(id, plan::text, 'real_estate')
...
BEGIN
  FOR v_table IN
    ...
    FROM (VALUES
      ...
      ('real_estate.real_estate_installments'),
      ('real_estate.real_estate_payments')
    ) AS module_tables(table_name)
  LOOP
...
DO $$
BEGIN
  IF to_regclass('real_estate.real_estate_transactions') IS NOT NULL THEN
    DROP TRIGGER IF EXISTS enforce_workspace_currency_plan_access ON real_estate.real_estate_transactions;
    CREATE TRIGGER enforce_workspace_currency_plan_access
      BEFORE INSERT OR UPDATE ON real_estate.real_estate_transactions
